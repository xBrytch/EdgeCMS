...
function GetLast($a){
		if(!empty($a) || !$a == ''){
			if(is_numeric($a)){
				...
				if($difference <= '59'){ $echo = ''. $difference .' secondes'; }
				elseif($difference <= '3599' && $difference >= '60'){ 
					...
					if($horas == 1) { $horas_str = 'heure'; }
					else { $horas_str = 'heures'; }
					$echo = ''.$horas.' '.$horas_str;
				}elseif($difference <= '518399' && $difference >= '86400'){
					$dias = floor(date('d', $difference));
					if($dias == 1) { $dias_str = 'journée'; }
					else { $dias_str = 'journées'; }
					$echo = ''.$dias.' '.$dias_str;
				}elseif($difference <= '2678399' && $difference >= '518400'){
					$semana = floor(date('d', $difference) / 7).'';
					if($semana == 1) { $semana_str = 'semaine'; }
					else { $semana_str = 'semaines'; }
					$echo = ''.floor($semana).' '.$semana_str;
				}else { $echo = ''.floor(date('m', $difference)).' mois'; }
				return $echo;
			}else{ return $a; }
		}else{ return 'aucune information de date'; }
	}
